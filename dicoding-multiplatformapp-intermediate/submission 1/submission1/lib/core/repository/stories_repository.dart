...
import 'package:submission1/core/dto/story_response.dart';
...
class StoriesRepository {
  ...
  Future<AddStoryResponse> addStories(
    ...
    if (response.statusCode == 201) {
      ...
      return AddStoryResponse.fromJson(responseData);
    } else {
  ...
  Future<StoriesListResponse> getListStories() async {
    final token = await getToken();
    Map<String, String> header = {"Authorization": "Bearer $token"};
    final response = await get(Uri.parse("$baseUrl/stories"), headers: header);
    if (response.statusCode == 200) {
